...
# set audio files
in_style = {
    'acapella': mixer.Sound('audio/masego-in-style/in_style-acapella.wav'),
    'melody': mixer.Sound('audio/masego-in-style/in_style-melody.wav'),
    'beat': mixer.Sound('audio/masego-in-style/in_style-beat.wav')
    }
...
try:
    while True: # program loop
        ...
        in_style['beat'].play()
        # melody.play(in_style['melody'])
        # beat.play(in_style['beat'])
        if GPIO.input(11):
            if in_style['acapella'].get_volume() > 0:
                in_style['acapella'].set_volume(0)
            else: 
                in_style['acapella'].set_volume(0.3)
            sleep(0.2)
        elif GPIO.input(13):
            if in_style['melody'].get_volume() > 0:
                in_style['melody'].set_volume(0)
            else: 
                in_style['melody'].set_volume(0.3)
            sleep(0.2)
        elif GPIO.input(15):
            if in_style['beat'].get_volume() > 0:
                in_style['beat'].set_volume(0)
            else: 
                in_style['beat'].set_volume(0.3)
            sleep(0.2)
